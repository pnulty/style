...
TEXT_FIELDS = ("text", "story", "content", "prompt")
...
def load_detectrl(base_dir: Path) -> list[NormalizedExample]:
    data_dir = base_dir / "detectrl"
...
def iter_rows(data_dir: Path, split: str) -> Iterator[dict[str, object]]:
    ...
    if csv_path.exists():
        ...
        return
    raise FileNotFoundError(f"No DetectRL data found for split '{split}'.")
